import { data } from "react-router";

function parseOptionalNumber(value) {
  if (value == null || value === "") return undefined;
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : undefined;
}

function parseOptionalBoolean(value) {
  if (value == null || value === "") return undefined;
  if (typeof value === "boolean") return value;

  const normalized = String(value).trim().toLowerCase();
  if (normalized === "true" || normalized === "1") return true;
  if (normalized === "false" || normalized === "0") return false;
  return undefined;
}

function getFilenameFromUrl(imageUrl, contentType) {
  try {
    const pathname = new URL(imageUrl).pathname;
    const name = pathname.split("/").pop();
    if (name) return name;
  } catch {
    // no-op
  }

  if (contentType?.includes("png")) return "shopify-image.png";
  if (contentType?.includes("webp")) return "shopify-image.webp";
  return "shopify-image.jpg";
}

function isValidImageUrl(url) {
  try {
    const parsed = new URL(url);
    return parsed.protocol === "https:" || parsed.protocol === "http:";
  } catch {
    return false;
  }
}

async function buildCropRequestPayload(formData, fetchImpl) {
  const uploadedFiles = formData.getAll("file").filter((item) => item instanceof Blob);
  const imageUrl = (formData.get("imageUrl") || "").toString().trim();
  const filesToCrop = [...uploadedFiles];

  if (imageUrl) {
    if (!isValidImageUrl(imageUrl)) {
      return {
        errorResponse: data(
          {
            error: "imageUrl must be an http(s) URL.",
            errorCode: "INVALID_IMAGE_URL",
            errorDetails: imageUrl,
          },
          { status: 400 },
        ),
      };
    }

    let imageResponse;
    try {
      imageResponse = await fetchImpl(imageUrl);
    } catch (error) {
      return {
        errorResponse: data(
          {
            error: "Failed to fetch image from imageUrl.",
            errorCode: "IMAGE_URL_FETCH_FAILED",
            errorDetails: error?.message || imageUrl,
          },
          { status: 400 },
        ),
      };
    }

    if (!imageResponse.ok) {
      return {
        errorResponse: data(
          {
            error: `Failed to fetch image from imageUrl (HTTP ${imageResponse.status})`,
            errorCode: "IMAGE_URL_FETCH_FAILED",
            errorDetails: imageUrl,
          },
          { status: 400 },
        ),
      };
    }

    const contentType = imageResponse.headers.get("content-type") || "application/octet-stream";
    const imageBlob = await imageResponse.blob();
    const normalizedBlob = new Blob([imageBlob], { type: contentType });

    filesToCrop.push(
      new File([normalizedBlob], getFilenameFromUrl(imageUrl, contentType), {
        type: contentType,
      }),
    );
  }

  if (!filesToCrop.length) {
    return {
      errorResponse: data(
        {
          error: "No images supplied. Upload a file or provide imageUrl.",
          errorCode: "NO_IMAGES",
        },
        { status: 400 },
      ),
    };
  }

  const cropOptions = {
    method: (formData.get("method") || "auto").toString(),
    pipeline: (formData.get("pipeline") || "auto").toString(),
    targetAspectRatio: parseOptionalNumber(formData.get("targetAspectRatio")),
    marginTop: parseOptionalNumber(formData.get("marginTop")),
    marginRight: parseOptionalNumber(formData.get("marginRight")),
    marginBottom: parseOptionalNumber(formData.get("marginBottom")),
    marginLeft: parseOptionalNumber(formData.get("marginLeft")),
    anchorHint: (formData.get("anchorHint") || "").toString().trim() || undefined,
    filters: (formData.get("filters") || "").toString().trim() || undefined,
    useHeadRotationHeuristic: parseOptionalBoolean(formData.get("headRotationHeuristicEnabled")),
    useCenterBiasHeuristic: parseOptionalBoolean(formData.get("centerBiasHeuristicEnabled")),
    overrideImageSizeLimit: parseOptionalBoolean(formData.get("overrideImageSizeLimit")),
  };

  return {
    filesToCrop,
    cropOptions,
  };
}

export function createCropAction({ cropImagesWithOutputs, authenticateAdmin, isPreviewRequest, fetchImpl = fetch }) {
  return async function cropAction({ request }) {
    try {
      if (!isPreviewRequest(request)) {
        await authenticateAdmin(request);
      }

      const formData = await request.formData();
      const payload = await buildCropRequestPayload(formData, fetchImpl);

      if (payload.errorResponse) {
        return payload.errorResponse;
      }

      const responsePayload = await cropImagesWithOutputs(payload.filesToCrop, payload.cropOptions);
      return data(responsePayload);
    } catch (err) {
      return data(
        {
          error: err.message || "Crop failed",
          errorCode: err.code || null,
          errorDetails: err.details || null,
        },
        { status: 500 },
      );
    }
  };
}

export const __testables__ = {
  buildCropRequestPayload,
};
