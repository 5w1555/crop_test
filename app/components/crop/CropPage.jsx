...
import { PLAN_CONFIG } from "../../utils/plan.js";
...
import { useCropPreview } from "./useCropPreview.js";
...
function getUnexpectedResponseMessageFromDiagnostics(diagnostics) {
  return mapDiagnosticsToErrorMessage(diagnostics);
}
...
export default function CropPage() {
  ...
  const { previewCropRect, updatePreviewCropRect: setPreviewCropRect, clearPreviewCropRect } = useCropPreview();
  ...
  const { state: workflowState, isBusy: isSubmittingCrop, beginSubmit, acceptSubmit, finishSuccess, finishFailure } = useCropWorkflow();
  const [storeUpdateResult, setStoreUpdateResult] = useState(null);
  const cropFailureSummary = workflowState.status === "failure" ? workflowState.error?.message || "Unable to crop image." : "";
  const cropFailureDetails = workflowState.status === "failure" ? workflowState.error?.technicalDetails || null : null;
    ...
    async (
      ...
      while (!isDone) {
        ...
        if (!statusResponse.ok) {
          ...
          const pollError = new Error(mapDiagnosticsToErrorMessage(statusDiagnostics));
        ...
        if (!jobStatus || typeof jobStatus !== "object") {
          ...
          const malformedError = new Error(
            getUnexpectedResponseMessageFromDiagnostics(statusDiagnostics),
          );
  ...
  useEffect(() => {
    ...
    const resumePendingCrop = async () => {
      ...
      try {
        ...
        const storeUpdated = { mode: "store-updated", ...normalizedResult };
        setStoreUpdateResult(storeUpdated);
        finishSuccess(storeUpdated);
        ...
        finishFailure({ message: error instanceof Error ? error.message : "Unable to resume crop job.", technicalDetails });
        ...
        if (!cancelled) {
          // workflow state is finalized in success/failure transitions
        }
      }
    };
  ...
  const handleCropSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const submitError = new Error(mapDiagnosticsToErrorMessage(responseDiagnostics));
      ...
      if (!responsePayload || typeof responsePayload !== "object") {
        ...
        const malformedError = new Error(
          getUnexpectedResponseMessageFromDiagnostics(responseDiagnostics),
        );
        malformedError.technicalDetails = buildErrorTechnicalDetails({
          correlationId,
          phase: "submit",
      ...
      // finalized through reducer transitions
    }
  };
